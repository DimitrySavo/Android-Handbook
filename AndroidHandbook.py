...
def submit_review():
    ...
    new_review = reviewClass.Review(username, email, review, rating_value, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def submit_topic():
    ...
    with open(FILE_PATH_TOPICS, "w") as file:
        topics_dict = [topic.to_dict() for topic in topics]#Проходит по списку объектов и превращает из в словарь
        json.dump(topics_dict, file)#Сериализует все объекты
